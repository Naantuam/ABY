...
export default function MaintenanceList() {
  ...
  const mapRecordToFrontend = (record) => ({
    ...
    Quantity: record.quantity, // "150 Units"
  ...
  const filteredItems = itemsWithBalance.filter((item) => {
    ...
    // --- INCOME FILTER ---
    const itemIncome = Number(item.Income);
    const incomeMin = filters.incomeMin ? Number(filters.incomeMin) : null;
    const incomeMax = filters.incomeMax ? Number(filters.incomeMax) : null;

    let matchIncome = true;
    if (incomeMin && incomeMax) {
      matchIncome = itemIncome >= incomeMin && itemIncome <= incomeMax;
    } else if (incomeMin && !incomeMax) {
      matchIncome = itemIncome === incomeMin;
    }

    // --- EXPENDITURE FILTER ---
    const itemExpenditure = Number(item.Expenditure);
    const expenditureMin = filters.expenditureMin ? Number(filters.expenditureMin) : null;
    const expenditureMax = filters.expenditureMax ? Number(filters.expenditureMax) : null;

    let matchExpenditure = true;
    if (expenditureMin && expenditureMax) {
      matchExpenditure = itemExpenditure >= expenditureMin && itemExpenditure <= expenditureMax;
    } else if (expenditureMin && !expenditureMax) {
      matchExpenditure = itemExpenditure === expenditureMin;
    }
    ...
    try {
      ...
      const newItems = data.map((row, index) => {
        ...
        return {
          ...
          Rate: row["Rate"] || "",
          Bal: row["Balance"] !== undefined ? Number(row["Balance"]) : (income - expenditure),
        };
      });
  ...
  const handleFieldChange = (id, field, value) => {
    setItems(items.map(item => {
      if (item.id === id) {
        ...
        if (['Income', 'Expenditure', 'Rate', 'Quantity'].includes(field)) {
                ...
                <td className="px-2 py-2">
                  {editingRowId === item.id ? (
                    <input
                      type="number"
                      value={item.Bal}
                      onChange={(e) => handleFieldChange(item.id, "Bal", e.target.value)}
                      className="border rounded px-2 py-1 w-full"
                    />
                  ) : (
                    fmt(item.Bal)
                ...
                <td className="px-2 py-2 flex gap-1">
                  {editingRowId === item.id ? (
                    <>
                      ...
                      <button
                        onClick={() => { setEditingRowId(null); setOriginalItem(null); fetchMaintenance(); }}
        ...
        {[...filteredItems].sort((a, b) => Number(b.id) - Number(a.id)).slice(0, visibleCount).map((item) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs mb-3">
              <div className="flex justify-between"><span className="text-gray-500">Qty:</span> <span className="font-semibold">{item.Quantity || "-"}</span></div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={itemsWithBalance.find(it => it.id === editingRowId)?.Quantity || ""}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    <option value="">Min</option>
                    {[...new Set(items.map((it) => it.Expenditure))].map(
                      (val, idx) => (
                        <option key={idx} value={val}>
                          {val.toLocaleString()}
                        </option>
