...
export default function EquipmentCard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Match mapping to verified API response
        setStats({
          total: data.total_equipment || 0,
          available: data.available_equipment || 0,
          active: data.active_equipment || 0,
          repair: data.repair_equipment || 0,
          retired: data.retired_equipment || 0,
          loading: false
        });
      } catch (error) {
  ...
  return (
    <div className="w-full bg-gray-50 flex justify-center items-center py-3">
      <div
        onClick={() => navigate('/admin/equipment')} // Assuming this is the correct route
